                            ...
                            {steps.map((s, i) => (
                                <React.Fragment key={s.id}>
                                    <button
                                        ...
                                        className={`flex items-center text-xs font-semibold uppercase tracking-wider transition-colors ${step === s.id ? 'text-[#6B63E8]' :
                                                s.id < step ? 'text-slate-900 cursor-pointer hover:text-[#6B63E8]' : 'text-slate-400 cursor-not-allowed'
                                            }`}
                                    >
                                        <span className={`w-5 h-5 rounded-full flex items-center justify-center mr-2 text-[10px] ${step === s.id ? 'bg-[#6B63E8]/10 text-[#6B63E8]' :
                                            ...
                                            {resumes.map(r => {
                                                ...
                                                return (
                                                    <div
                                                        ...
                                                        className={`p-5 border rounded-xl cursor-pointer transition-all flex items-center justify-between ${isSelected
                                            ...
                                            <div className="flex gap-3 bg-slate-50 p-1.5 rounded-xl border border-slate-200">
                                                {['Professional', 'Confident', 'Concise'].map(tone => (
                                                    <button
                                                        ...
                                                        className={`flex-1 py-2 text-sm font-medium rounded-lg transition-all ${formData.tone === tone ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'
                                                            }`}
                                                    >
                                ...
                                <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                        {Object.keys(TEMPLATES).map((tmpl, index) => {
                                            ...
                                            const PreviewComponent = TEMPLATES[tmpl as keyof typeof TEMPLATES] as React.ComponentType<any>;
                                            return (
                                                <div
                                                    key={tmpl}
                                                    onClick={() => setFormData({ ...formData, templateId: tmpl })}
                                                    className={`cursor-pointer rounded-xl overflow-hidden border-2 transition-all group ${isSelected ? 'border-[#6B63E8] shadow-md ring-2 ring-[#6B63E8]/20' : 'border-slate-200 hover:border-slate-300 hover:shadow-sm'
                                                        }`}
                                                >
